...
namespace UIDisable
{
    internal class Main : MelonMod
    {
        ...
        private static void DisableUI()
        {
            ...
            if (!Save.Settings.ComboEnabled) pnlCommonUI.gameObject.SetActive(false);

            if (!Save.Settings.ScoreEnabled) pnlBattleOthers.GetChild(5).gameObject.SetActive(false);

            if (!Save.Settings.BottomBarEnabled) pnlBattleOthers.GetChild(3).gameObject.SetActive(false);

            if (!Save.Settings.HitPointEnabled) sceneObjectController.transform.GetChild(1).gameObject.SetActive(false);

            if (!Save.Settings.ComboEnabled) GameObject.Find("MultiHitComboControl").SetActive(false);

            if (!Save.Settings.EffectEnabled) GameObject.Find("Effects").SetActive(false);

            if (!Save.Settings.PauseButtonEnabled) pnlBattleOthers.GetChild(2).gameObject.SetActive(false);
        }
